...
public class StarFieldTemplate implements EntityGroupTemplate {
	...
	private static final Rectangle REGION = new Rectangle(-1600, -240, 3200,
			480);
	...
	public Array<Entity> buildEntities(EntityWorld world, Object... args) {
		...
		for (int i = 0; i < BIG_PLANETS; i++) {
			...
			Vector2 position = getSafePosition(xRange, yRange,
					BIG_PLANET_WIDTH, BIG_PLANET_HEIGHT);

			int type = furthestUsedPlanet
					+ rand.nextInt(BIG_PLANET_TYPES / BIG_PLANETS) + 1;
		...
		for (int i = 0; i < SMALL_PLANETS; i++) {
			...
			int type = furthestUsedPlanet
					+ rand.nextInt(SMALL_PLANET_TYPES / SMALL_PLANETS) + 1;
		...
		for (int i = 0; i < BIG_STARS; i++) {
			...
			Vector2 position = getSafePosition(xRange, yRange, BIG_STAR_WIDTH,
					BIG_STAR_HEIGHT);
	...
	private Vector2 getSafePosition(float xRange, float yRange, int boundWidth,
			int boundHeight) {
		...
		while (searching) {
			...
			Rectangle spot = new Rectangle(position.x - boundWidth / 2,
					position.y - boundHeight / 2, boundWidth, boundHeight);

			boolean safe = true;

			for (Rectangle usedSpot : usedSpots) {
				if (spot.overlaps(usedSpot))
					safe = false;
			}
